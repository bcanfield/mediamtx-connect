...
export function StreamCard({
  ...
  // `recordState` is the settled query, so between the write and the refetch it
  // still reads the old value and a slow toggle looks like a no-op. Render what
  // we asked for instead: the invalidation above is awaited before the success
  // dispatch, so `isPending` covers the write and the refetch with no gap. A
  // failure flips it false and the card falls back to the settled value.
  const askedFor = updatePathConfig.isPending ? updatePathConfig.variables?.conf.record : undefined
  const shownRecordState: RecordState = askedFor === undefined
    ? recordState
    : askedFor ? 'on' : 'off'
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 rounded-lg">
              ...
              <DropdownMenuItem
                ...
                disabled={recordState === 'unknown' || updatePathConfig.isPending}
              >
