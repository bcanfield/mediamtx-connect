...
- A production build: `pnpm build` at the repo root first — `run.sh` launches
  `node apps/api/dist/server.mjs`, which only exists after a build.
...
# 5. from repo root: `pnpm build` (once), then `node apps/api/dist/server.mjs` with the env below
...
gen-clips.sh ──> clips/*.mp4 ──(ffmpeg -stream_loop)──> MediaMTX  <── seed-recordings.sh writes
   (synthetic          publish-streams.sh                 │            .work/recordings + .work/screenshots
    camera footage)                                       │
                                                          ...
                                                          │ API  :9998
                                                          ▼
                                              Hono app (:3100) ── reads config/paths, serves pages,
                                              isolated config store (.work/data)   serves recordings from disk
                                                          ▲                         │
...
| App | 3100 | 3000 | `run.sh` (`PORT=3100`, `node apps/api/dist/server.mjs`) + `DEMO_BASE_URL` |

App env (`run.sh` exports these for the server):

```
DATA_DIR=.work/data                        # isolated config store — never touches your real one
PORT=3100
NODE_ENV=production
...
How Connect builds the two URLs that must line up (from `apps/web/src/features/streams/`):
...
`run.sh` wipes `.work/data` each run; the app's JSON config store seeds itself
from the env above on first boot, so the UI points at the demo MediaMTX with
zero manual config.
...
(`pnpm i18n:check`): all 30 `docs/i18n/README.*.md` must be re-synced in the
