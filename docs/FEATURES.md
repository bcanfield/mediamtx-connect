...
- **Honest state when the path resolves to nothing** — a wildcard-backed path that isn't publishing has no runtime path to name its wildcard entry and no entry under its own name (both lookups 404), which is reachable by bookmarking or typing the URL for a stream that has since stopped. `getPathConfig` reports that as its own state (`not-publishing`) rather than as a failure, and the page renders "This stream isn't publishing" with a link to path defaults (§3.3) — the settings the stopped path still inherits. The generic "Invalid Config" message is now reserved for a MediaMTX that can't be reached. Resolving effective config for a stopped wildcard-backed path would mean reimplementing MediaMTX's own regex matching and stays out of scope (`docs/debt/20260716153634-path-config-needs-runtime-path.md`). `packages/contract/src/index.ts` (`PathConfigStateSchema`), `apps/api/src/router.ts` (`getPathConfig`), `apps/web/src/features/mediamtx-config/path-config-page.tsx`
...
| `config.mediamtx.getPathConfig` | query | Resolves one path's effective config through its `confName`, or reports `not-publishing` when neither lookup lands; `null` when unreachable. |
...
- **`PathConfigStateSchema`** — discriminated union for what a path config lookup found: `resolved` (`EffectivePathConfigSchema` plus the discriminant) or `not-publishing` (§3.4).
