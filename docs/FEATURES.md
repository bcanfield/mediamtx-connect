...
- **Per-path detail + editor** — one path's own settings, reached from its row in the catalog (§3.5) and from a stream's card ("Edit path config"). Same Recording and Path Hooks sections as path defaults: both scopes are the same key set, one applied server-wide and one to a single path (ADR 0002). `apps/web/src/features/mediamtx-config/path-config-page.tsx`, `apps/web/src/features/mediamtx-config/sections.ts` (`PATH_CONFIG_SCOPE`)
- **Inherited vs overridden, per value** — every field carries an INHERITED or OVERRIDDEN marker, from comparing its value against the path-defaults scope read alongside it (`v3/config/pathdefaults`). MediaMTX resolves defaults into the entry it serves, so every key reads back filled in and "unset" is not a state the API can report — whether the value still *matches* its default is. The marker follows the live form value, so a field flips to OVERRIDDEN as it is edited, and a path that sets a key to the value its default already holds reads as inherited (indistinguishable over the API). Nothing is marked while path defaults can't be read: no comparison is not the same answer as nothing overridden. The scopes that inherit from nothing — global (§3.2) and path defaults (§3.3) — pass no defaults and show no markers. `apps/web/src/features/mediamtx-config/config-field-row.tsx` (`InheritanceMarker`), `apps/web/src/features/mediamtx-config/mediamtx-config-form.tsx` (`inheritedValues`)
