...
- **Codec ↔ protocol compatibility matrix** — above the form, a row per read protocol (RTSP / RTMP / HLS / WebRTC / SRT) against a column per codec the path is publishing, each row verdicted **Plays** (green) / **Partial** (amber) / **Won't play** (red), with a plain-language reason naming the codecs responsible. MediaMTX routes one source to many readers and never re-encodes, so a track the reader's protocol can't mux is dropped rather than converted — an H264 + AAC stream reads Partial on WebRTC (whose audio set is Opus/G722/G711, so a browser gets picture and no sound), and an M-JPEG camera reads Won't play everywhere but RTSP. Cells never rest on colour: the tick is `aria-hidden` and each carries a visually-hidden "{protocol} carries/drops {codec}". `apps/web/src/features/mediamtx-config/codec-compat-panel.tsx`
- **The table is version-pinned, and says so** — the support rules are transcribed from MediaMTX **1.19.3**'s own per-protocol codec tables (`docs/4-read/*`), and the panel's note names that version, because the answer moves between releases (Enhanced RTMP brought AV1/VP9/H265 to RTMP). RTSP is `'any'` rather than a list — MediaMTX forwards RTP as it arrives, which its docs state outright — so it is the one row that is always green. Low-Latency HLS gets no column of its own: it is the same HLS muxer under `hlsVariant`, so its codec set is identical, and the note says so instead of duplicating five cells. `apps/web/src/lib/codec-compat.ts` (`MEDIAMTX_CODEC_TABLE_VERSION`)
- **Codecs only while a source is connected** — MediaMTX knows a path's tracks only while something publishes to it, so an idle path renders "no tracks to check" rather than a matrix computed off an empty mix, which would read green everywhere and claim a stream that isn't there plays fine. The tracks ride out on `getPathConfig`, which already calls `v3/paths/get` for the `confName` — the matrix costs no extra MediaMTX request. `apps/api/src/router.ts` (`getPathConfig`), `packages/contract/src/index.ts` (`PathConfigResultSchema`)
- **One remedy, linked not automated** — when any row isn't green, a link to MediaMTX's remuxing/re-encoding docs sits under the table. Auto-remediation beyond that link is deliberately out of scope.
...
- **Live read/write through MediaMTX HTTP API** — `config.mediamtx.getPathConfig` (`v3/paths/get` + `v3/config/paths/get`) + `config.mediamtx.updatePathConfig` (`v3/config/paths/add`|`patch`) + `config.mediamtx.deletePathConfig` (`v3/config/paths/delete`). `apps/api/src/router.ts`, `apps/api/src/mediamtx.ts`
...
- **`codec-compat`** — MediaMTX 1.19.3's documented codec support per read protocol as one table, plus `compatFor(codecs)`, which turns a path's tracks into a `plays` / `partial` / `blocked` verdict per protocol and the codecs behind it (§3.4). `apps/web/src/lib/codec-compat.ts`
...
- **Dark / Light theme** — persisted to `localStorage` via the in-app `ThemeProvider`, dark default. Toggled by `ModeToggle` (single-click, View Transitions API circle-reveal). Beyond the shadcn tokens, the Geist extensions carry the app's status colours — `--link`, `--success`, `--warning`, `--live` — each given a darkened value in light mode and a brightened one in dark so small text clears 4.5:1 on either background. `apps/web/src/globals.css`
...
| `config.mediamtx.getPathConfig` | query | Resolves one path's effective config through its `confName` — `{status: 'resolved', confName, conf, codecs}`, `{status: 'unresolved'}` when there is no runtime path and no entry of its own, `null` when unreachable. `codecs` are the runtime path's current tracks, off the same `paths/get` (§3.4). |
...
| `config.mediamtx.deletePathConfig` | mutation | Deletes a path's own entry so it tracks its wildcard again; throws on failure. |
...
- **`EffectivePathConfigSchema` / `PathConfigResultSchema`** — `{confName, conf}` under a `resolved` / `unresolved` union; the resolved branch also carries `codecs`, the runtime path's current tracks, empty whenever nothing is publishing (§3.4).
...
- **`path-config-page.test.tsx`** — revert to inherited (§3.4): no affordance while the path tracks a wildcard entry, one once it has its own, `config.mediamtx.deletePathConfig` called with the path name only after the confirm, and nothing sent when the dialog is cancelled. Plus the `unresolved` empty state: the title and path-defaults link, and the absence of "Invalid Config", the subheader and the revert action. Plus the codec matrix (§3.4): five green rows for a mix every protocol carries, the partial verdict naming *which* codec WebRTC drops rather than only tinting a cell, a blocked row reading "nothing to serve" (with the re-encode link) instead of partial, and an idle path reading "no tracks to check" rather than a green matrix over an empty mix. `apps/web/src/features/mediamtx-config/path-config-page.test.tsx`
- **`codec-compat.test.ts`** — the compatibility table as a pure function in the `logic` project (§3.4): RTSP carrying codecs no other protocol takes, AV1 over RTMP (Enhanced RTMP), VP8 dropped by RTMP/SRT but kept by WebRTC, AAC dropped only by WebRTC, G711 dropped by HLS/SRT, the per-mix verdicts including the H264 + AAC row, blocked vs partial, a repeated codec collapsing to one column, and an empty mix answering nothing at all. Its own module because asserting a transcribed table through a rendered page tests the renderer, not the transcription. `apps/web/src/lib/codec-compat.test.ts`
...
- **Harness** — `src/test/render.tsx` mirrors `main.tsx`'s provider stack (QueryClient, `IntlProvider` with shipped English messages, memory-history router, `ThemeProvider`, `Toaster`) and awaits `router.load()`; `src/test/rpc-server.ts` serves `/rpc/*` through MSW backed by the **real `RPCHandler`** over an `implement(contract)` stub router, so a contract change fails these at typecheck. `renderWithProviders` also takes `realI18n` to swap the fixed English `IntlProvider` for the app's own `I18nProvider`, for the suites that are *about* locale switching. Two Vitest projects (`logic` = node, `component` = happy-dom) in `apps/web/vitest.config.ts`. **270 Vitest tests total** — 83 api + 181 web + 6 scripts. `docs/adr/0005-fast-test-suite.md`
