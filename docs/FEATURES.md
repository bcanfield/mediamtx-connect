...
- **Live health panel** — a read-only panel above the editor showing what the path is *doing*, not what it is configured to do: uptime (from `readyTime`), reader count, bytes received/sent, `inboundFramesInError`, a chip per track, and the publisher's and readers' session types. The stored config alone never says whether a camera is actually working. It refreshes every 5 s off a `refetchInterval` on the query itself, so leaving the page unmounts the query and the polling stops with it. Resolution rides on `tracks2`'s codec properties; a server that only serves the older `tracks` string list shows codecs without one rather than nothing. `apps/web/src/features/mediamtx-config/path-health-panel.tsx`, `apps/api/src/router.ts` (`getPathHealth`)
- **Idle is not an error** — a path MediaMTX isn't running (`v3/paths/get` → 404) and one it is running but that isn't `ready` both resolve to `{status: 'idle'}`: nothing is flowing, which is a state and not a failure. A server that didn't answer stays `null` and says so, because silence says nothing about whether the path is up. The panel is independent of the config read below it, so a path whose config can't be resolved (the unresolved empty state) still reports its health. `apps/api/src/router.ts` (`getPathHealth`), `packages/contract/src/index.ts` (`PathHealthSchema`)
...
- **Live read/write through MediaMTX HTTP API** — `config.mediamtx.getPathConfig` (`v3/paths/get` + `v3/config/paths/get`) + `config.mediamtx.updatePathConfig` (`v3/config/paths/add`|`patch`) + `config.mediamtx.getPathHealth` (`v3/paths/get`) + `config.mediamtx.getPathConnections` (`v3/paths/get`) + `config.mediamtx.deletePathConfig` (`v3/config/paths/delete`). `apps/api/src/router.ts`, `apps/api/src/mediamtx.ts`
...
| `/config/mediamtx/paths/{name}` | One path's detail — live health, plus effective config marked inherited vs overridden | `apps/web/src/features/mediamtx-config/path-config-page.tsx` |
...
| `config.mediamtx.getPathHealth` | query | One path's runtime health — `{status: 'live', readyTime, publisher, readers, tracks, bytesReceived, bytesSent, framesInError}`, `{status: 'idle'}` when it isn't running or isn't ready, `null` when unreachable. Polled by the detail page's health panel (§3.4). |
...
- **`PathHealthSchema` / `PathTrackSchema`** — runtime health for one path, as a `live` / `idle` union: counters, session types and a `{codec, resolution}` per track. `framesInError` is nullable because MediaMTX versions before it existed send no counter, and that is not the same answer as zero; `resolution` is nullable for the same reason plus audio tracks, which have none (§3.4).
