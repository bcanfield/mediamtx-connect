...
- **The card renders the flip while it's in flight** — the footer REC indicator and the menu's ON/OFF both show what was asked for from the click until the server answers, so a slow write doesn't read as a no-op. Derived from the mutation's own `variables` while `isPending` (the invalidation runs in `onSuccess`, so `isPending` covers the write *and* the `streams.list` refetch), not from an optimistic cache write — a failure flips `isPending` false, the card snaps back to the settled state with nothing to roll back, and the error still toasts. The menu item is disabled while the write is in flight, so a second click can't compute its flip target from the pre-write state and race the first. In-flight is not a fourth record state: `unknown` means MediaMTX's state is unreadable, whereas in flight we know exactly what we asked for. `apps/web/src/features/streams/stream-card.tsx`
