...
- **`path-config-page.test.tsx`** — revert to inherited (§3.4): no affordance while the path tracks a wildcard entry, one once it has its own, `config.mediamtx.deletePathConfig` called with the path name only after the confirm, and nothing sent when the dialog is cancelled. Plus the `unresolved` empty state: the title and path-defaults link, and the absence of "Invalid Config", the subheader and the revert action. `apps/web/src/features/mediamtx-config/path-config-page.test.tsx`
