...
- **PR title check** (`.github/workflows/pr-title.yml`) — `main` is squash-merged, so the PR title is the commit subject semantic-release parses; this rejects titles that aren't conventional commits. Convention documented in `CONTRIBUTING.md` § PR titles.
