...
- **The card shows what it asked for while the write is in flight** — the footer REC indicator and the menu label flip on click and the item disables until the write *and* its `streams.list` refetch have both landed, rather than rendering the settled query throughout. Otherwise a slow write reads as a no-op, and a menu reopened inside the round trip computes its flip off the stale value and races two writes to opposing values. Derived from the mutation's own `variables`/`isPending` — no cache write, so a failure toasts and the card falls back to whatever MediaMTX still reports with no rollback path to get wrong. `unknown` gains no fourth state: it means the server's state is unreadable, while in-flight means we know exactly what we asked for. `apps/web/src/features/streams/stream-card.tsx`
