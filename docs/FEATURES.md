...
- **Live read through MediaMTX HTTP API** — `config.mediamtx.listPaths` joins `v3/config/paths/list` against `v3/paths/list` in one failure domain: without either half a row would be missing or would claim idle. Read-only — creating, editing and deleting entries are §3.4's per-path surface. `apps/api/src/router.ts`, `apps/api/src/mediamtx.ts` (`configPathsList`)
...
- **Hand-rolled typed client** for the endpoints the app uses: `v3/paths/{list,get}`, `v3/config/global/{get,patch}`, `v3/config/pathdefaults/{get,patch}`, `v3/config/paths/{list,get,add,patch,delete}` (fetch-based, shapes mirror MediaMTX v1.11.3 swagger). `apps/api/src/mediamtx.ts`
...
| `/config/mediamtx/paths` | Read-only catalog of MediaMTX path config entries | `apps/web/src/features/mediamtx-config/paths-catalog-page.tsx` |
