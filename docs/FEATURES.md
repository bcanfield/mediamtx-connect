...
- **`path-config-page.test.tsx`** — revert to inherited (§3.4): no affordance while the path tracks a wildcard entry, one once it has its own, `config.mediamtx.deletePathConfig` called with the path name only after the confirm, and nothing sent when the dialog is cancelled. Plus the `unresolved` empty state: the title and path-defaults link, and the absence of "Invalid Config", the subheader and the revert action. Plus the inheritance markers (§3.4): a field matching path defaults reads INHERITED while one that differs reads OVERRIDDEN, the `record` section switch carries a marker of its own, a field flips to OVERRIDDEN as it is typed into, and nothing is marked when path defaults can't be read. `apps/web/src/features/mediamtx-config/path-config-page.test.tsx`
