...
- **Designed connection states** (per the design handoff, board 1c) — server-unreachable renders a centered red-tinted panel with the MediaMTX URL in mono, "Open App Config" + "Retry" buttons, and a visible auto-retry countdown (15 s); zero streams renders a dashed panel with publish-URL hints (RTSP/RTMP/SRT) in a mono code well, their ports built from the server's own listen addresses by the shared `publishTargets` (§1.2.3) rather than hardcoded and a disabled protocol left out entirely (the well is dropped when none are enabled); missing playback URL renders an amber banner above the grid with cards at 75 % opacity and Play disabled. `apps/web/src/features/streams/live-view-states.tsx`
...
- **Record state indicator** — a card's footer shows "· ● REC" whenever the stream is *effectively* recording: its own override merged over path defaults, as MediaMTX resolves it. Inherited `on` is the stock setup, so a card reading only the stream's own (usually absent) entry would claim OFF while MediaMTX writes files (ADR 0002). A config entry the api couldn't read renders as its own third state — an amber "· ○ REC?" — rather than as OFF, which would make the same claim (§1.2.1). `apps/web/src/features/streams/stream-card.tsx`
...
#### 1.2.1 Record toggle
- **Per-stream record toggle** — the actions menu's "Record" item shows effective ON/OFF/UNKNOWN and flips it for that one stream, via `config.mediamtx.updatePathConfig` (§3.4). UNKNOWN disables the item: flipping a state we couldn't read would be a guess at what it's flipping. It writes the path's own `{record}` override only: path defaults are the other place `record` lives, and writing them from a card would start or stop recording for every stream on the server (ADR 0002). A wildcard-backed stream — the normal case — has no entry to patch, so the first toggle materializes a sparse one; the live session survives it, unlike a hook write. `apps/web/src/features/streams/stream-card.tsx`
- **In flight, the card shows what was asked for** — the footer REC indicator, the menu label and the next flip's target all read the value the write is carrying until the server answers, instead of the settled `streams.list` state, which would make a slow write read as a no-op and let a reopened menu compute a second write from a value already superseded. The item is disabled for that window too, matching "Take snapshot". Derived from the mutation rather than an optimistic cache write, so a failure needs no rollback: it toasts and the card snaps back to the settled state on its own. The window covers the refetch as well as the write — the `streams.list` invalidation runs in `onSuccess`, which TanStack Query awaits before clearing `isPending`, so there is no gap where the write has landed but the prop is still stale. `apps/web/src/features/streams/stream-card.tsx`
...
- **Record state is `on` / `off` / `unknown`** — each config-entry read has its own failure domain: an entry that 404s (deleted between the paths list and the read) or a read that fails resolves that entry's streams to `unknown`. Neither a confident `off` — which would claim a stream isn't recording while MediaMTX writes files — nor a whole grid blanked to "Can't reach MediaMTX", which is what a shared catch used to do when `paths/list` and the global conf had both answered. `packages/contract/src/index.ts` (`RecordStateSchema`), `apps/api/src/router.ts` (`streams.list`)
...
- **Snapshot mtimes are read outside the MediaMTX failure domain** — `streams.list` stats our own screenshots directory after the MediaMTX calls have already answered, so an unreadable directory (or a PNG the cleanup job unlinked mid-read) surfaces as a failed query rather than as "Can't reach MediaMTX" pointed at a healthy server. `apps/api/src/router.ts` (`streams.list`)
...
#### 1.2.3 Publish URLs
- **Copy publish URLs** — the actions menu's "Copy publish URLs" item copies this stream's publish URLs for the protocols the server serves (one per line) to the clipboard, then toasts success or, if the browser blocks clipboard access, failure. With every source protocol disabled there is nothing to copy and the item is disabled. `apps/web/src/features/streams/stream-card.tsx`
- **One builder, ports from config** — `publishTargets` builds the URLs from the server's own listen addresses (`rtspAddress`/`rtmpAddress`/`srtAddress` off global config), keeping only the port and pairing it with the browser-facing host, so an operator who changed a listen address is pointed at the right one. The same builder feeds the zero-streams hints (§1.1), so the copy action and the empty-state can never disagree on a port; MediaMTX defaults (8554/1935/8890) apply only when the server reports no address. A protocol whose global enable flag (`rtsp`/`rtmp`/`srt`) is `false` emits no target at all, so an operator never gets a URL that would silently refuse the publisher; the toggles default to on, so only an explicit `false` drops a target — a missing flag, or a global config the app couldn't read, keeps it. `apps/web/src/lib/publish.ts`
...
- **Loopback-only WebRTC hosts get named** — when MediaMTX advertises nothing but loopback (`127.0.0.1` / `localhost` / `::1`) in `webrtcAdditionalHosts` and the app is being read at a non-loopback hostname, an amber banner above the grid says so and deep-links to the WebRTC section of the global config (§3.2). Without it the half-configured deployment of §13.2 is silent: WHEP negotiates, ICE times out after 8 s, and every card quietly plays HLS. It claims the *config* is wrong rather than that playback is broken — bare-metal MediaMTX can still be reachable over an interface IP `webrtcIPsFromInterfaces` supplied, which the browser cannot see from here. An empty list never trips it (that setup works), nor does `webrtc: false` (nothing to fix). The per-card `WEBRTC UNAVAILABLE` pill stays LOW-LAT-only (§1.3.1) — this is a once-per-deployment fact, not a per-card one. `apps/web/src/lib/playback.ts` (`advertisesOnlyLoopback`), `apps/web/src/features/streams/live-view-states.tsx` (`WebrtcHostBanner`)
...
- **Section deep-link** — `?section=` lands the page on one section rather than at the top, the same search param the per-path page takes (§3.4); the live view's loopback-WebRTC banner (§1.3.1) is this route with `?section=webrtc`. `apps/web/src/main.tsx`, `apps/web/src/features/mediamtx-config/mediamtx-config-form.tsx` (`initialSection`)
...
### 3.4 MediaMTX path detail — `/config/mediamtx/paths/$name`
- **Per-path detail + editor** — one path's own settings, reached from its row in the catalog (§3.5) and from a stream's card ("Edit path config"). A Source section, then the same Recording and Path Hooks sections as path defaults: those two scopes are the same key set, one applied server-wide and one to a single path (ADR 0002). Every field is editable in place and saved through the shared rail form's save bar. `apps/web/src/features/mediamtx-config/path-config-page.tsx`, `apps/web/src/features/mediamtx-config/sections.ts` (`pathConfigScope`)
- **`source` is on this scope only** — where a stream comes from belongs to one path, and MediaMTX doesn't serve it from pathdefaults, so it rides on `PathConfigSchema` (the path-defaults key set plus `source`) and appears on no other config page. Editing it writes the path's own override the same way every other key does. `packages/contract/src/index.ts` (`PathConfigSchema`), `apps/web/src/features/mediamtx-config/sections.ts` (`SOURCE_SECTION`)
- **Source validation mirrors the server** — MediaMTX takes `publisher`, `redirect`, `rpiCamera` or a URL in one of ten schemes it can pull from (`rtsp`, `rtsps`, `rtmp`, `rtmps`, `http`, `https`, `udp`, `srt`, `whep`, `wheps`); anything else is a refused PATCH, which is a slow way to learn about a typo. The rule lives in the contract next to the schema and the form fails the field against it on blur, before any request goes out. It is deliberately *not* enforced on the wire: MediaMTX stays the authority on what it accepts, and a source kind added in a later version still has to read back. `packages/contract/src/index.ts` (`isValidPathSource`), `apps/web/src/features/mediamtx-config/sections.ts` (`pathConfigScope`)
- **A rejected save lands on the field that caused it** — MediaMTX answers a refused write with `{"error": "…"}` naming the key it disliked (`invalid source: '…'`), which `updatePathConfig` forwards as a `BAD_REQUEST` instead of flattening to "failed". The form matches that reason against the keys the save actually sent — longest first, so `record` can't swallow `recordPath` — and sets it as that field's message; a reason naming none of them falls back to the toast alone. `apps/api/src/router.ts` (`updatePathConfig`), `apps/web/src/features/mediamtx-config/server-rejection.ts` (`offendingField`), `apps/web/src/features/mediamtx-config/mediamtx-config-form.tsx`
- **Inherited vs overridden, per value** — every field carries an INHERITED or OVERRIDDEN marker, from comparing its value against the path-defaults scope read alongside it (`v3/config/pathdefaults`). MediaMTX resolves defaults into the entry it serves, so every key reads back filled in and "unset" is not a state the API can report — whether the value still *matches* its default is. The marker follows the live form value, so a field flips to OVERRIDDEN as it is edited, and a path that sets a key to the value its default already holds reads as inherited (indistinguishable over the API). Nothing is marked while path defaults can't be read, nor for a key that scope doesn't hold at all (`source`): no comparison is not the same answer as nothing overridden. The scopes that inherit from nothing — global (§3.2) and path defaults (§3.3) — pass no defaults and show no markers. `apps/web/src/features/mediamtx-config/config-field-row.tsx` (`InheritanceMarker`), `apps/web/src/features/mediamtx-config/mediamtx-config-form.tsx` (`inheritedValues`)
- **Path hooks** — the 15 `runOn*` commands MediaMTX fires on a path's own lifecycle (`runOnInit`, `runOnDemand*`, `runOnReady`/`runOnNotReady`, `runOnRead`/`runOnUnread`, `runOnRecordSegmentCreate`/`Complete`), in a Path Hooks section on both path scopes. The server-wide `runOnConnect`/`runOnDisconnect` fire per client connection and stay on the global page (§3.2) — they are the only genuinely global hooks. `apps/web/src/features/mediamtx-config/sections.ts` (`PATH_SECTIONS`), `packages/contract/src/index.ts` (`PathDefaultsSchema`)
- **Restart warning on the hooks section** — an amber callout under the Path Hooks header on both path scopes: saving a hook restarts the path and disconnects its publisher (verified on v1.19.2; a `record*` save does not), which the identical save bars otherwise hide. Carried by the section descriptor (`warnsOnSave`), so a future disruptive section opts in the same way. `apps/web/src/features/mediamtx-config/sections.ts` (`PATH_SECTIONS`), `apps/web/src/features/mediamtx-config/mediamtx-config-form.tsx` (`ConfigSection`)
...
- **Honest empty state for a name with nothing to resolve** — when there is no runtime path under the name *and* no config entry of its own, both lookups 404 and `getPathConfig` reports `{status: 'unresolved'}` rather than the `null` that means "MediaMTX didn't answer". The page then renders a read-only dashed-border block — no subheader, no revert action, no form — with a link to path defaults. The copy claims neither route into the state (a stream that stopped publishing, or a name that was never a path), because MediaMTX cannot tell them apart: with a wildcard entry present, any name is potentially valid. It also does not try to resolve effective config, which would mean reimplementing MediaMTX's wildcard matching (`docs/debt/20260716153634-path-config-needs-runtime-path.md`). `apps/web/src/features/mediamtx-config/path-config-page.tsx` (`UnresolvedPathConfig`), `apps/api/src/router.ts` (`getPathConfig`), `packages/contract/src/index.ts` (`PathConfigResultSchema`)
- **Revert to inherited** — a header action, shown only once the path has an entry of its own, that deletes that entry behind a confirm dialog (`config/paths/delete`) and returns the path to tracking the wildcard that covers it. Without it a materialize is one-way: a single save pins the path to its own entry and every override it holds stays there. The form remounts on the way back, so it shows the inherited values rather than the deleted ones. `apps/web/src/features/mediamtx-config/path-config-page.tsx`, `apps/api/src/router.ts` (`deletePathConfig`)
- **Delete path** — a destructive header action beside the revert, shown on the same condition (there is an entry of its own to delete), that removes the entry behind a confirm dialog and lands on the catalog (§3.5). It is the same `config/paths/delete` the revert runs, for the other operator intent: getting rid of the path rather than of its overrides. MediaMTX cannot say which of the two a given delete will be — that depends on whether a wildcard entry still covers the name, which needs its own wildcard matching to answer (`docs/debt/20260716153634-path-config-needs-runtime-path.md`) — so the confirm names both outcomes instead of picking one. `apps/web/src/features/mediamtx-config/path-config-page.tsx` (`DeletePath`)
- **The confirm names what the delete would cut off** — opening the dialog reads the runtime path (`config.mediamtx.getPathConnections`) and, when a publisher or readers are attached, renders an amber callout naming the publisher's session type and the readers' — MediaMTX drops an entry with live sessions on it without a word, so this is the only warning there is. The confirm button stays disabled until that read lands, or the warning could be clicked past before it appeared; a read that fails says so rather than letting silence imply the path is idle. `apps/web/src/features/mediamtx-config/path-config-page.tsx` (`DeletePath`), `apps/api/src/router.ts` (`getPathConnections`)
- **Live read/write through MediaMTX HTTP API** — `config.mediamtx.getPathConfig` (`v3/paths/get` + `v3/config/paths/get`) + `config.mediamtx.updatePathConfig` (`v3/config/paths/add`|`patch`) + `config.mediamtx.getPathConnections` (`v3/paths/get`) + `config.mediamtx.deletePathConfig` (`v3/config/paths/delete`). `apps/api/src/router.ts`, `apps/api/src/mediamtx.ts`

### 3.5 MediaMTX paths catalog — `/config/mediamtx/paths`
- **Catalog of config entries** — a Paths nav tab listing one table row per entry MediaMTX holds (`v3/config/paths/list`): name, `source`, and whether it is live. This is the *configured* population, not the runtime one the live grid shows (§1.1) — a wildcard-backed path appears on neither its own row nor at all, and one regex entry stands in for every path it matches (ADR 0002). `apps/web/src/features/mediamtx-config/paths-catalog-page.tsx`, `apps/web/src/components/app-header.tsx`
- **Row opens the path's detail page** — the name cell is a link to `/config/mediamtx/paths/{name}` (§3.4), URL-encoded so a regex entry's name survives the trip. The catalog is the only way in to a path that has no card on the live grid. `apps/web/src/features/mediamtx-config/paths-catalog-page.tsx`
- **Regex badge** — a `~`-prefixed name is a regular expression, so it gets a REGEX badge and a tooltip saying the entry backs every path name it matches. `apps/web/src/features/mediamtx-config/paths-catalog-page.tsx`
- **Live, not just present** — an entry reads LIVE only while a runtime path it backs is `ready`. Every static entry always has a runtime path, idle or not, so "has a runtime path" would mark all of them live and say nothing; a regex entry is never a runtime path itself and is live when any path naming it as `confName` is. `apps/api/src/router.ts` (`listPaths`)
- **Empty / error states** — an unreachable server renders the same red `StatusPanel` as the live view (mono address, "Open App Config" + "Retry") off a typed `connection-error` branch, so a server that didn't answer never renders as a server with no paths; a genuinely empty catalog renders a dashed panel pointing at path defaults, where every stream's settings then come from. `apps/web/src/features/mediamtx-config/paths-catalog-page.tsx`, `packages/contract/src/index.ts` (`PathCatalogStateSchema`)
- **Credentials masked in the source column** — a `source` that carries `user:password@` renders with the password replaced by a fixed-width mask, so the URL the guided add composed (§3.6) isn't read back out in a table nobody had to open. The username stays legible — it says which account the path connects as — and the mask is fixed-width, so it doesn't leak the length either. `apps/web/src/features/mediamtx-config/rtsp-source.ts` (`maskSourceCredentials`)
- **Live read through MediaMTX HTTP API** — `config.mediamtx.listPaths` joins `v3/config/paths/list` against `v3/paths/list` in one failure domain: without either half a row would be missing or would claim idle. Editing and deleting entries are §3.4's per-path surface; creating one is §3.6. `apps/api/src/router.ts`, `apps/api/src/mediamtx.ts` (`configPathsList`)

### 3.6 Guided RTSP add — `/config/mediamtx/add-path`
- **Source URL builder, not a text box** — the overwhelmingly common case (pointing MediaMTX at an RTSP camera) as a form over the URL's *parts* — host, port, stream path, username, password, transport — with the finished `source` composed from them. Reached from an "Add RTSP path" action in the catalog header (§3.5). `apps/web/src/features/mediamtx-config/add-rtsp-path-page.tsx`
- **Credentials percent-encoded** — camera passwords routinely contain `@`, `:` and `/`. Written raw into a URL, the first `@` ends the authority in the wrong place and MediaMTX pulls from a host that isn't the camera; the composer escapes both halves of the credential pair. A username with no password composes as `user@`, not `user:@`. `apps/web/src/features/mediamtx-config/rtsp-source.ts` (`composeRtspSource`)
- **Password never echoed** — the field is `type="password"`, the live preview of the URL about to be written shows the password masked, and the catalog row it lands on masks it too (§3.5). `apps/web/src/features/mediamtx-config/add-rtsp-path-page.tsx`
- **Transport is a sparse override** — the four `rtspTransport` values (`automatic` / `udp` / `multicast` / `tcp`) as a segmented control. `automatic` sends no key at all: it is what an entry without one already does, and the entry stays a sparse override of path defaults (ADR 0002). `packages/contract/src/index.ts` (`RTSP_TRANSPORTS`)
- **A rejected create says why** — MediaMTX answers a refused write with `{"error": "..."}` (a name already in use, a source it can't parse). That reason rides along on the thrown `MediaMtxError` and comes back as the message of a `BAD_REQUEST`, which the page renders in an error panel beside the form rather than as a toast that disappears with the field still wrong. `apps/api/src/mediamtx.ts` (`MediaMtxError`), `apps/api/src/router.ts` (`addPath`)
- **Lands on the new path playing** — on success the wizard navigates to `/?play={name}`, so the first thing after creating a path is the picture from it. `apps/web/src/features/mediamtx-config/add-rtsp-path-page.tsx`
- **Route is a sibling of the catalog** — `/config/mediamtx/add-path`, not `/config/mediamtx/paths/new`, which would reserve `new` as a path name and shadow a real MediaMTX path so named — the same reasoning as `path-defaults` (ADR 0002). `apps/web/src/main.tsx`
- **Live write through MediaMTX HTTP API** — `config.mediamtx.addPath` → `v3/config/paths/add`. `add`, never `updatePathConfig`'s materialize-or-patch: a name already in use has to fail rather than quietly overwrite the path behind it. **Out of scope, each a follow-up:** the other source kinds (RPi camera, WHEP-redirect, `publisher`, always-available file). `apps/api/src/router.ts`
...
- **Env / Config drift warning** — on subsequent boots, for each bootstrap env var that the operator has *explicitly* set in the environment (raw `process.env` via `rawEnv` in `apps/api/src/env.ts`, distinct from schema defaults), if its value differs from the corresponding stored config value a single structured `WARN` log lists each mismatch and reminds the operator that env only seeds the first boot. Since `docker-compose.yml` now sets `REMOTE_MEDIAMTX_URL` explicitly (§13.2), it is always in `rawEnv` under compose — so changing the playback URL in `/config` makes this warn on every boot. That is correct: `REMOTE_MEDIAMTX_URL` and MediaMTX's advertised WebRTC host have to move together, and nothing else watches them. It went from a warning that essentially never fired to one that routinely does.
- **Live snapshot cron** — every 30 seconds (`*/30 * * * * *`) lists MediaMTX paths and, for each `ready` one, grabs a single frame off the RTSP feed (`rtsp://<mediaMtxUrl host>:<rtspAddress port>/<path>`) into `<screenshotsDirectory>/<streamName>/live.png`. MediaMTX exposes no snapshot endpoint, so this wraps the protocol it already serves. Written via tmp+rename so `/latest` never serves a half-written file; each `ffmpeg` is SIGKILLed after 15s so a stalled camera can't pile up processes. Also runs once on boot. This is what backs the card's "first capture in ~30s" copy — snapshots do **not** require recording to be enabled. The per-stream captures and the on-demand `captureSnapshot` (§1.2.4) acquire a **shared concurrency gate** (`MAX_CONCURRENT_CAPTURES`) before spawning and release it on ffmpeg exit, so the two sources are bounded together rather than either running unbounded (`docs/debt/20260715154503-snapshot-cron-unbounded-ffmpeg.md`).
- **Thumbnail generation cron** — every 30 minutes (`*/30 * * * *`) scans the recordings tree for MP4s without a sibling PNG and spawns `ffmpeg -ss 00:00:00 -i <file>.mp4 -frames:v 1 <file>.png`. Parallel but bounded: each extraction acquires a **sibling concurrency gate** (`MAX_CONCURRENT_THUMBNAILS`) and releases it on ffmpeg exit, so a large recordings backlog runs in waves instead of spawning every ffmpeg at once. The gate is a sibling of the capture one, not the same gate — the backlog is batch work and would otherwise queue ahead of the live cron and a user-triggered snapshot (`docs/debt/20260715154503-snapshot-cron-unbounded-ffmpeg.md`). Also runs once on boot. Backs the per-recording thumbnails in the recordings list.
- **Both caps derived from CPU count** — `MAX_CONCURRENT_CAPTURES` and `MAX_CONCURRENT_THUMBNAILS` are each `os.availableParallelism()` clamped to 2–8, replacing the flat `4`: a 2-core NAS no longer runs four ffmpeg at once and an 8-core host is no longer serialized down to four. The clamp bounds are still reasoned rather than benchmarked (`docs/debt/20260717153914-snapshot-cap-untuned.md`).
...
- **Geist type ramp as theme tokens** — the nine sub-`text-base` sizes the design handoff uses are named `@theme` tokens rather than arbitrary values at each call site: `text-section` (15 px headings), `text-body` (13.5 px, also the `<body>` size), `text-control` (13 px buttons/inputs/tabs), `text-row` (12.5 px dense rows), `text-lead` (12 px), `text-meta` (11.5 px help + metadata), `text-status` (11 px mono), `text-label` (10.5 px uppercase eyebrows), `text-micro` (10 px pills), plus `tracking-title` (−0.02 em). Size only — no paired line-height — and Tailwind's own `text-sm`/`lg`/`xl` are untouched. `cn()` extends tailwind-merge so these dedupe against Tailwind's scale. `apps/web/src/globals.css`, `apps/web/src/lib/utils.ts`
...
- **README translation staleness guard** — `docs/i18n/.translation-status.json` records the source `README.md` hash plus per-locale "last synced at hash" entries. `scripts/readme-i18n-check.mjs` (run via `pnpm i18n:check`) fails CI when the source hash drifts from any locale's recorded hash or when a locale README file is missing. It also compares each locale README's structure against the source — heading outline, per-fenced-block language and line count, and every backticked literal — so a lockstep hash bump can't pass off stale content as synced.
...
| `/config/mediamtx/paths` | Catalog of MediaMTX path config entries | `apps/web/src/features/mediamtx-config/paths-catalog-page.tsx` |
| `/config/mediamtx/add-path` | Guided RTSP add — source URL builder | `apps/web/src/features/mediamtx-config/add-rtsp-path-page.tsx` |
| `/config/mediamtx/paths/{name}` | One path's detail — effective config, marked inherited vs overridden | `apps/web/src/features/mediamtx-config/path-config-page.tsx` |
...
| `streams.list` | query | MediaMTX paths (with effective record state — `on`/`off`/`unknown`) + hlsAddress + remote URL, or a typed `connection-error` state reserved for the `paths/list` + global-conf reads (§1.2.1). |
...
| `config.mediamtx.listPaths` | query | Every MediaMTX path config entry (`{name, source, isRegex, active}`), joined against the runtime paths for liveness, or a typed `connection-error` state (§3.5). |
| `config.mediamtx.addPath` | mutation | Creates a path config entry from a composed `source` (+ optional `rtspTransport`); a rejected create throws `BAD_REQUEST` carrying MediaMTX's own reason (§3.6). |
| `config.mediamtx.getPathConfig` | query | Resolves one path's effective config through its `confName` — `{status: 'resolved', confName, conf}`, `{status: 'unresolved'}` when there is no runtime path and no entry of its own, `null` when unreachable. |
...
| `config.mediamtx.getPathConnections` | query | What is attached to a runtime path right now — `{publisher, readers}` by session type, `null` when unreachable. Read by the delete confirm (§3.4). |
| `config.mediamtx.deletePathConfig` | mutation | Deletes a path's own entry — the path tracks its wildcard again, or is gone if none covers it; throws on failure. |
...
- **`StreamSchema`** — `{name, readyTime, recordState, codecs, viewers, snapshotMtime}`, the element type of `StreamsStateSchema`'s connected branch. `recordState` is `RecordStateSchema` — effective state, the path's own override merged over path defaults (§1.2.1). `codecs`/`viewers` come straight off the polled path list; `snapshotMtime` is ours, not MediaMTX's (§1.2.2).
- **`RecordStateSchema`** — `'on' | 'off' | 'unknown'`. `unknown` is a first-class state, not a default: a config entry the api couldn't read says nothing about whether MediaMTX is writing files (§1.2.1).
- **`StreamsStateSchema`** — discriminated union for the live view connection state.
- **`PathCatalogEntrySchema` / `PathCatalogStateSchema`** — `{name, source, isRegex, active}` per config entry, wrapped in the same `connection-error` / `connected` union as the live view so an unreachable server can't read as an empty catalog (§3.5).
- **`PathConnectionsSchema`** — `{publisher, readers}` for one runtime path, both by MediaMTX session type (`rtspSession`, `webRTCSession`, …). The reader ids MediaMTX pairs them with are opaque and name nothing an operator would recognise, so they stop at the contract boundary (§3.4).
...
- **`webrtcAdditionalHosts` is commented out, owned by compose** — in Docker, MediaMTX gathers ICE candidates from its own container interfaces, which a browser on the host cannot route to; WHEP then negotiates successfully and never connects, and the player falls back to HLS. Advertising a routable host fixes that, and the value comes from `REMOTE_MEDIAMTX_HOST` via `MTX_WEBRTCADDITIONALHOSTS` (§13.2), not from this file. The key is commented out on purpose rather than left live: MediaMTX's env override always beats the config file, so an operator who edits the value here and restarts would be silently overridden by a compose default they never saw. **Accepted regression:** a bare `docker run` mounting this file *without* our compose now advertises no host and loses localhost WHEP until the line is uncommented — the commented line and its note are the whole mitigation. Mirrored in `mediamtx.dev.yml`. `mediamtx.yml`
...
- **`jobs.test.ts`** — the live snapshot cron: only `ready` paths are captured, paths without a name are skipped, the RTSP url is built from the configured host + `rtspAddress` port (with an `:8554` fallback), tmp+rename on ffmpeg exit 0, tmp discarded on failure, a stalled ffmpeg SIGKILLed at 15s, and no kill for one that already exited. Plus the shared concurrency cap (§1.2.4): the cron never runs more ffmpeg at once than the cap, an on-demand `captureSnapshot` is counted against the same cap, a spawn that both errors and closes releases only one slot, and on-demand capture resolves on ffmpeg success / rejects on failure. Plus `generateScreenshots` (§4): ffmpeg runs only for recordings missing a thumbnail, the thumbnail gate caps concurrent spawns and lets a queued one through on exit, error+close releases one slot, and a live capture is not queued behind a thumbnail backlog. Sibling modules are mocked with factories; timers are faked and both gates are reset between cases. `apps/api/src/jobs.test.ts`
...
- **`router.test.ts`** — `streams.list` record state: an inherited `true` surfaces as recording when only path defaults enable it (the stock setup), a stream with its own entry reports that entry's state, and the whole grid costs one `configPathGet` per distinct `confName` rather than one per stream. `streams.snapshot` delegates to `captureSnapshot` for the named stream and surfaces a capture failure as an error. `config.mediamtx.getPathConfig` resolves a publishing path through the `confName` its runtime path reports, reports `unresolved` when neither lookup lands, and returns `null` only when MediaMTX is unreachable. `config.mediamtx.listPaths` flags `~`-prefixed entries, calls an entry active only while a path it backs is `ready` (a regex entry through the paths naming it as `confName`), and reports `connection-error` rather than an empty catalog. `config.mediamtx.addPath` writes the composed source with the chosen transport, leaves `rtspTransport` off when the wizard sent none, passes MediaMTX's own reason through on a rejected create, and claims no reason when the server gave none. `config.mediamtx.getPathConnections` names the publisher and every reader by session type, reports an idle path for a name MediaMTX isn't running, and returns `null` — not idle — when MediaMTX is unreachable. `apps/api/src/router.test.ts`
...
- **`mediamtx.test.ts`** — the MediaMTX client over a stubbed `fetch`: every read hits `{mediaMtxUrl}:{mediaMtxApiPort}/v3<route>` and returns the parsed body, path names are percent-escaped into the URL, writes carry their method + `application/json` + the serialized override (and `configPathDelete` no body at all), a PATCH/DELETE/204 answer is not parsed as JSON (MediaMTX replies to a write with an empty body), a non-ok response throws naming method, route and status, a transport failure propagates, and 404 is an answer of `null` — but only for the per-path reads, which is what a wildcard-backed path relies on (ADR 0002). `apps/api/src/mediamtx.test.ts`
- **`recordings-fs.test.ts`** — the recordings/screenshots helpers against a real temp tree: `summarizeStreamRecordings` keys off subdirectories only (a loose file beside them is not a stream) and reports each one's file count and newest mtime; `listStreamRecordingFiles` sorts newest-first and hides the dotfile of an in-progress segment; `screenshotUrlFor` swaps the recording's extension for `.png` and answers null when that thumbnail was never generated; `latestScreenshotPathFor` prefers `live.png` over a thumbnail that would outsort it, falls back to the newest PNG by name, and answers null for a missing directory or one holding no PNG; the URL and mtime helpers follow it, and both escape the stream name. A missing recordings or stream directory throws rather than reading as empty. `apps/api/src/recordings-fs.test.ts`
...
- **`stream-card.test.tsx`** — the card's rendered state from props: stream name + play affordance, idle vs `online since`, codec chips present/absent, viewer count shown at zero and pluralized at one and omitted when unknown, snapshot pill with and without an age, the thumbnail-error empty state, the REC badge on/off, and Stop-vs-Play driven by `?play`. `apps/web/src/features/streams/stream-card.test.tsx`
...
- **`paths-catalog-page.test.tsx`** — the paths catalog (§3.5): one row per entry with its source, the REGEX badge and LIVE marker on a live regex entry and their absence on a plain idle one, the unreachable panel rendering *instead of* the catalog or the empty state, the empty state's path-defaults link, the masked password on a source that carries credentials, the name cell linking to the path's detail page, and the header's guided-add link. `apps/web/src/features/mediamtx-config/paths-catalog-page.test.tsx`
- **`add-rtsp-path-page.test.tsx`** — the guided RTSP add (§3.6): the source composed from its parts with the password percent-encoded, a chosen transport sent and an automatic one left off, the landing on `/?play={name}`, the preview masking the password, the password never rendering as readable text, MediaMTX's rejection reason surfacing on the page, and a submit with no path name being refused before it reaches the API. `apps/web/src/features/mediamtx-config/add-rtsp-path-page.test.tsx`
- **`rtsp-source.test.ts`** — the source builder in isolation: host/port/stream-path composition, credentials placed before the host, the percent-encoding of a username and password that would otherwise cut the URL, a username with no password, a normalized leading slash, and the mask hiding the password at a fixed width while keeping the username and leaving a credential-free source alone. `apps/web/src/features/mediamtx-config/rtsp-source.test.ts`
- **`path-config-page.test.tsx`** — revert to inherited (§3.4): no affordance while the path tracks a wildcard entry, one once it has its own, `config.mediamtx.deletePathConfig` called with the path name only after the confirm, and nothing sent when the dialog is cancelled. Plus path deletion (§3.4): no affordance while the path tracks a wildcard entry, an idle path deleted and the page landed on the catalog, a publisher and readers named in the confirm before anything is sent, the confirm held disabled until that read lands, and nothing sent when the dialog is cancelled. Plus the `unresolved` empty state: the title and path-defaults link, and the absence of "Invalid Config", the subheader and both destructive actions. Plus the inheritance markers (§3.4): a field matching path defaults reads INHERITED while one that differs reads OVERRIDDEN, the `record` section switch carries a marker of its own, a field flips to OVERRIDDEN as it is typed into, and nothing is marked when path defaults can't be read. `apps/web/src/features/mediamtx-config/path-config-page.test.tsx`
...
- **`live-view-page.test.tsx`** — the loopback-WebRTC banner's wiring (§1.3.1) against a stubbed global conf and a document moved off localhost: the warning and its `?section=webrtc` link on a loopback-only advertisement, and its absence for a browser on that same machine, for a list that also holds a routable host, and for `webrtc: false`. `apps/web/src/features/streams/live-view-page.test.tsx`
...
- **Harness** — `src/test/render.tsx` mirrors `main.tsx`'s provider stack (QueryClient, `IntlProvider` with shipped English messages, memory-history router, `ThemeProvider`, `Toaster`) and awaits `router.load()`; `src/test/rpc-server.ts` serves `/rpc/*` through MSW backed by the **real `RPCHandler`** over an `implement(contract)` stub router, so a contract change fails these at typecheck. `renderWithProviders` also takes `realI18n` to swap the fixed English `IntlProvider` for the app's own `I18nProvider`, for the suites that are *about* locale switching. Two Vitest projects (`logic` = node, `component` = happy-dom) in `apps/web/vitest.config.ts`. **265 Vitest tests total** — 118 api + 141 web + 6 scripts. `docs/adr/0005-fast-test-suite.md`
...
- **ESLint 10 + `@antfu/eslint-config`** (lint + format in one tool) with custom rules:
  ...
  - **No conditionals in a Playwright `test()` body** (`tests/e2e/**`) — a guarded assertion passes when the feature is broken. Cleanup guards in `afterEach` stay allowed. `docs/adr/0005-fast-test-suite.md`
...
- **GitHub Actions CI** (`.github/workflows/ci.yml`) — lint, typecheck, i18n guards, build, Playwright E2E against a real MediaMTX with fake streams, Docker image smoke test. Its conclusion on `main` is the signal the release train gates on.
