...
- **Honest empty state for a name with nothing to resolve** — when there is no runtime path under the name *and* no config entry of its own, both lookups 404 and `getPathConfig` reports `{status: 'unresolved'}` rather than the `null` that means "MediaMTX didn't answer". The page then renders a read-only dashed-border block — no subheader, no delete action, no form — with a link to path defaults. The copy claims neither route into the state (a stream that stopped publishing, or a name that was never a path), because MediaMTX cannot tell them apart: with a wildcard entry present, any name is potentially valid. It also does not try to resolve effective config, which would mean reimplementing MediaMTX's wildcard matching (`docs/debt/20260716153634-path-config-needs-runtime-path.md`). `apps/web/src/features/mediamtx-config/path-config-page.tsx` (`UnresolvedPathConfig`), `apps/api/src/router.ts` (`getPathConfig`), `packages/contract/src/index.ts` (`PathConfigResultSchema`)
- **Delete path** — a header action, shown only once the path has an entry of its own, that deletes that entry behind a confirm dialog (`config/paths/delete`) and lands on the catalog (§3.5), where the row it removed used to be. One action rather than two, because MediaMTX has one call: with the entry gone the path falls back to whichever wildcard entry covers the name — undoing a materialize, which would otherwise be one-way — and where none does, MediaMTX stops serving the path. The confirm says both, since MediaMTX won't say which wildcard would cover a name (`docs/debt/20260716153634-path-config-needs-runtime-path.md`). `apps/web/src/features/mediamtx-config/path-config-page.tsx` (`DeletePath`), `apps/api/src/router.ts` (`deletePathConfig`)
- **The confirm names what the delete would cut off** — opening it reads `v3/paths/get` for the *runtime* path (`config.mediamtx.getPathConnections`) and lists the publisher and every reader by MediaMTX's own kind (`rtspSession`, `webRTCSession`, `hlsMuxer`) in an amber block, one line each, with the reader count. MediaMTX will delete a path out from under a live publisher without complaint, so the read is the guard: the confirm button stays disabled until it lands. A read that fails is its own answer (`unreadable`) and warns that this may cut off a live stream — rendering it as "nothing connected" is the confirmation an operator clicks straight through. `apps/web/src/features/mediamtx-config/path-config-page.tsx` (`DeletePath`), `apps/api/src/router.ts` (`getPathConnections`), `packages/contract/src/index.ts` (`PathConnectionsSchema`)
- **Live read/write through MediaMTX HTTP API** — `config.mediamtx.getPathConfig` (`v3/paths/get` + `v3/config/paths/get`) + `config.mediamtx.updatePathConfig` (`v3/config/paths/add`|`patch`) + `config.mediamtx.deletePathConfig` (`v3/config/paths/delete`) + `config.mediamtx.getPathConnections` (`v3/paths/get`). `apps/api/src/router.ts`, `apps/api/src/mediamtx.ts`
...
| `config.mediamtx.getPathConnections` | query | What deleting a path would cut off — `{status: 'read', publisher, readers}` by MediaMTX connection kind, or `{status: 'unreadable'}` when the server didn't say (§3.4). |
...
- **`PathConnectionsSchema`** — `{status: 'read', publisher, readers}` | `{status: 'unreadable'}`, what a delete would cut off. `unreadable` is a state rather than an empty pair, so a server that didn't answer can't render as a path with nothing connected (§3.4).
...
- **`path-config-page.test.tsx`** — path deletion (§3.4): no affordance while the path tracks a wildcard entry, one once it has its own, `config.mediamtx.deletePathConfig` called with the path name only after the confirm and the catalog reached afterwards, nothing sent when the dialog is cancelled, the publisher and reader count named on a path with connections, the `unreadable` warning when MediaMTX won't say, and neither shown for an idle path. Plus the `unresolved` empty state: the title and path-defaults link, and the absence of "Invalid Config", the subheader and the delete action. Plus the inheritance markers (§3.4): a field matching path defaults reads INHERITED while one that differs reads OVERRIDDEN, the `record` section switch carries a marker of its own, a field flips to OVERRIDDEN as it is typed into, and nothing is marked when path defaults can't be read. `apps/web/src/features/mediamtx-config/path-config-page.test.tsx`
...
- **`path-config.spec.ts`** — the per-path page: effective config inherited from the wildcard entry, path-scoped hooks only, `?section=pathHooks` deep link, a hook write, a save that materializes a sparse entry without restarting the session, and a delete through the page that removes that entry again — naming the live publisher in the confirm first, and landing on the catalog — which is also how the spec cleans up, so it holds no raw `config/paths/delete` of its own. Plus the unresolved empty state on a name that is neither running nor configured, which needs no fixture setup at all.
