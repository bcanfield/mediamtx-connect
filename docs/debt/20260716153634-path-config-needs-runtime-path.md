...
principal: 4h
interest: +1 unresolvable page per operator who opens path config for a stream that stopped publishing
...
getPathConfig resolves a path's confName from /v3/paths/get/<name>, which only knows paths that exist at runtime. For a wildcard-backed stream that isn't currently publishing, that 404s, the handler falls back to the path's own name, and that entry 404s too — there are no inherited settings to show.
...
Issue #209 stopped that case reading as a broken app: the handler reports it as its own state (not-publishing) and the page says the stream isn't publishing and links to path defaults, instead of the generic "Invalid Config". The principal here is what's left — actually resolving the settings a stopped wildcard-backed path would inherit, which still needs that regex matching.
