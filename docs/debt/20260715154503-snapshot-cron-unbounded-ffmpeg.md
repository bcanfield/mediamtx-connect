...
**Resolved (2026-07-28, issue 204).** Every ffmpeg spawn in `jobs.ts` is now bounded. `generateScreenshots` acquires `thumbnailGate` (`MAX_CONCURRENT_THUMBNAILS`) per recording and releases it on exit, so a backlog runs in waves. It is a **sibling** gate, not the capture one: the recordings sweep is batch work and hundreds of queued MP4s ahead of the 30s cron and the on-demand snapshot would starve exactly the spawns a user is waiting on. Both gates come from one `createSpawnGate` factory, so the acquire/release semantics (permit handed straight to the next waiter; release exactly once when a spawn both errors and closes) can't drift between them. Covered at the api unit-test seam (`jobs.test.ts`: thumbnail cap enforced, queued spawn released on exit, error+close releases one slot, a live capture not queued behind a thumbnail backlog).
