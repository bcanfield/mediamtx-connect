...
**Paid down 2026-07-27 (issue #207).** The Path Hooks section carries a `warnsOnSave` flag on its descriptor and renders an amber callout under its header on both path scopes: "Saving a hook restarts the path and disconnects its publisher — the stream drops until it reconnects." A component test (mediamtx-config-form.test.tsx) pins it to the two path scopes and out of Recording and the global Hooks section. MediaMTX's behavior is unchanged, so the e2e-hooks workaround stays; the record toggle still writes record* only and needs no warning.
