...
**Resolved (2026-07-28, issue 220).** The policy moved verbatim to its own file, `docs/RESPONSIVE.md`, discoverable from the `CLAUDE.md` "Where things live" table. Its own doc rather than a `docs/PROJECT-STRUCTURE.md` section: that doc scopes itself to "layout and naming" and its rules are about where files go, so a viewport-range/breakpoint-role policy would have been the same category mix one level down. No pointer was left behind in `CONTEXT.md` — a "see docs/RESPONSIVE.md for breakpoints" line is still non-glossary content, and would keep modelling the habit this entry is about. Nothing outside `CONTEXT.md` referenced the section, so no other links needed updating.
